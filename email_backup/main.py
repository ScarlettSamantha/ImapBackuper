...
def main():
    ...
    parser.add_argument('--daemon', type=bool, default=True, help='Switches between daemon mode and the default run once mode.')
    parser.add_argument('--no-resume', type=bool, default=True, help='If this switch is active it will not attempt to resume the latest download session.')
    ...
    backup.backup(args.imap_import_folder, bool(args.daemon))
